...
import { startSystemWorkoutTimer, stopSystemWorkoutTimer } from "@/lib/workout-timer";
...
export default function SessionScreen() {
  ...
  const toggleChrono = useCallback(() => {
    if (chronoRunning) {
      ...
      void stopSystemWorkoutTimer().catch(() => {});
    } else {
      // « Start » démarre toujours un nouveau chronomètre, même après un stop.
      const startedAt = Date.now();
      chronoBase.current = 0;
      chronoStart.current = startedAt;
      chronoTimer.current = setInterval(() => {
        setChronoMs(
          chronoBase.current +
            (chronoStart.current !== null ? Date.now() - chronoStart.current : 0),
        );
      }, 200);
      setChronoMs(0);
      setChronoRunning(true);
      void startSystemWorkoutTimer(startedAt, workout?.name ?? "Séance en cours").catch(() => {});
    }
  }, [chronoRunning, stopChronoInterval, workout?.name]);
  ...
  useEffect(() => {
    // Nettoie une activité système devenue orpheline après un redémarrage de l'app.
    void stopSystemWorkoutTimer().catch(() => {});
    return () => resetChrono();
  }, [resetChrono]);
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.emptyContent}>
          ...
          {templates.length === 0 && (
            <Text style={{ color: colors.textSecondary }}>Aucune routine pour le moment.</Text>
          )}
          {templates.map((t) => (
            <View
              key={t.id}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.keyboardAvoidingView}>
        ...
        <KeyboardAwareScrollView
          ...
          bottomOffset={chronoVisible ? 80 : 56}
        ...
        <View style={styles.footer}>
          <Pressable
            ...
            onPress={() =>
              router.push({ pathname: "/ajouter-exercice", params: { mode: "workout" } })
            }
          >
            <Text style={{ color: "#007AFF", fontWeight: "600" }}>+ Exercice</Text>
          </Pressable>
          <Pressable
            style={[styles.primaryButton, { backgroundColor: "#007AFF", flex: 1 }]}
            ...
            <View style={styles.chronoBar}>
              <View style={styles.chronoTitleRow}>
                ...
                <Pressable
                  ...
                  style={({ pressed }) => [
                    styles.chronoSummary,
                    pressed && styles.chronoSummaryPressed,
                  ]}
                >
                  <Text style={styles.chronoLabel} numberOfLines={1}>
                    Chronomètre
                  </Text>
              ...
              <Pressable
                style={({ pressed }) => [
                  styles.chronoButton,
                  pressed && styles.chronoButtonPressed,
                ]}
            ...
            <View style={styles.chronoToggleDock}>
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.chronoIcon,
                  styles.chronoToggle,
                  pressed && styles.chronoIconPressed,
                ]}
              >
                <Text style={styles.chronoIconText}>⏱</Text>
              </Pressable>
        ...
        <SafeAreaView style={styles.chronoExpandedScreen}>
          ...
          <View style={styles.chronoExpandedContent}>
            ...
            <Pressable
              accessibilityRole="button"
              style={({ pressed }) => [
                styles.chronoExpandedStartButton,
                pressed && styles.chronoButtonPressed,
              ]}
              onPress={toggleChrono}
            >
          ...
          <Pressable
            ...
            accessibilityLabel="Réduire le chronomètre"
            style={({ pressed }) => [
              styles.chronoCollapseButton,
              pressed && styles.chronoCollapseButtonPressed,
            ]}
            onPress={() => setChronoExpanded(false)}
          >
        ...
        <Pressable style={styles.modalBackdrop} onPress={() => setFinishOpen(false)}>
          <Pressable style={[styles.modalCard, { backgroundColor: colors.backgroundElement }]}>
            ...
            <Text style={[styles.modalMessage, { color: colors.textSecondary }]}>
              ...
              {templates.find((t) => t.id === workout?.template_id)?.name
                ? `« ${templates.find((t) => t.id === workout?.template_id)?.name} »`
                : ""}{" "}
              ?
            </Text>
